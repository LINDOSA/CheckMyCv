...
export async function GET(req: NextRequest) {
  try {
    ...
    const buffer = generatePdfBuffer(cvData);
    ...
    console.error('[API /api/export/pdf GET] Error generating PDF:', error);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[API /api/export/pdf POST] Error generating PDF:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to generate PDF document.' },
      { status: 500 }
    );
